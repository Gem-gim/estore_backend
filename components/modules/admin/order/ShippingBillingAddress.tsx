...
export default function ShippingBillingAddress({
  ...
  const handledelivered = () => {
    ...
    setLoading(true);
    axios
      .put(process.env.NEXT_PUBLIC_SERVER_URL + "/api/order", data)
      .then((response) => {
    ...
    <div className="w-full col-span-1 p-4 bg-neutral-100 lg:w-[320px] flex flex-col gap-10 items-flex-start pt-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <h1 className="text-xl font-bold">{order?.user.firstName}</h1>
      ...
      <div className="inline-flex gap-4">
        <Mail />
        <span className="text-slate-500">{order?.user.firstName}</span>
      </div>
