  ...
  const handleSubmit = async (e) => {
    ...
    if (!error && paymentMethod) {
      try {
        ...
        axios
          .post( process.env.NEXT_PUBLIC_SERVER_URL + `/api/order/stripe`, data)
          .then((response) => {
            const data = response.data;
            if (data.success) {
              // window.location.reload(false)
            }
            toast.custom(<CustomToast  message="Your payment has been done! Wait to be delivered!" status="success"/>);
