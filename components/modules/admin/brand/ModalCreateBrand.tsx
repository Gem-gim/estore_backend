...
export default function ModalCreateBrand({
  ...
  const handleSave = async (values: Brand) => {
    if (loading) {
      return;
    }
    const data = {
    ...
    setLoading(true);
    await axios
      .post(process.env.NEXT_PUBLIC_SERVER_URL + "/api/admin/brands", data)
      .then((response) => {
    ...
    <Dialog open={openModal}>
      <DialogTrigger asChild>
        <button
          ...
          className="p-2 inline-flex gap-4 items-center ms-10 text-xl text-white bg-indigo-400"
        >
          Add <PlusCircle />{" "}
        </button>
      </DialogTrigger>
